...
function emulateHackedCycle(ns, information) {
	...
	const victimHackingTime = calculateHackingTime(victim, player) - 50;
	...
	if (totalRequiredMemory > hackerMemoryAvailable) {
		...
		const hackerMemoryAvailableText = ns.nFormat(hackerMemoryAvailable, "0,0.00");
		errors.push("WARNING: Not enough memory to run all the hack cycle forever." +
			" The victim server required " +
			requiredMemoryText + " GiB, but the hacker server has " +
			hackerMemoryAvailableText + " GiB.");
	}
...
export async function main(ns) {
	/* global console*/
	const timelineDelayMs = 500;
	const addHome = true;

	const hackersUnfiltered = ns.getPurchasedServers();
	if (addHome) {
		hackersUnfiltered.push("home");
	}

	const hackers = hackersUnfiltered
		.sort(function (a, b) {
			const serverA = ns.getServer(a);
			const serverB = ns.getServer(b);
			return serverB.maxRam - serverA.maxRam ||
				serverB.cpuCores - serverA.cpuCores;
		});

	const victims = [];
	for (let hostname of jsGetVictimsFiltered(ns, true)) {
		victims.push({victim: hostname, hacker: "",});
	}

	// victims.push({victim: "n00dles", hacker: "",});
	// victims.push({victim: "sigma-cosmetics", hacker: "",});
	...
	for (let serverIndex = 0; serverIndex < serversMin; serverIndex += 1) {
		victims[serverIndex].hacker = hackers[serverIndex];
	...
	while (true) {
		nameOfActions.forEach(function (actionType) {
			// ns.tprint("actionType: \n" + JSON.stringify(actionType));
			const actions = execution[actionType];
			...
			for (let action of actions) {
				...
				if (currentTime >= action.timeNext) {
					...
					ns.exec(action.script, action.hacker, action.threads, action.victim, processId, startTime, action.timeNext, action.time, action.timeWait, action.timeDelay);
					// const pid = ns.exec(action.script, action.hacker, action.threads, action.victim, processId, startTime, action.timeNext, action.time, action.timeWait, action.timeDelay);
					// if (pid < 1) {
					// 	ns.tprint("WARNING: This process is not executed: " + JSON.stringify(action));
					// 	// return;
					// }
					action.timeNext += numberOfActions * timelineDelayMs;
					processId += 1;
				}
			}
		});
